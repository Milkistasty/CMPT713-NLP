...
print('now we are using', device, 'to train our model.')
...
def introduce_noise(sentence, aug_ratio=0.1):
    """
    This function introduces adversarial noise in a sentence.
    The noise types include swapping characters, replacing characters, deleting characters,
    and simulating keyboard typing errors.
    
    Arguments:
        sentence {list of strings} -- input sentence as list of words
        aug_ratio {float} -- ratio of words in the sentence to augment
    
    Returns:
        noisy_sentence {list of strings} -- sentence with noise introduced
    """
    ...
    for word in sentence:
        if random.random() < aug_ratio:
            ...
            if len(noisy_word) > 1:
                ...
                noise_type = random.choice(['swap', 'replace', 'delete', 'keyboard'])

                if noise_type == 'swap':
                    # Randomly swap two adjacent characters
                    idx = random.randint(0, len(noisy_word) - 2)
                    noisy_word[idx], noisy_word[idx + 1] = noisy_word[idx + 1], noisy_word[idx]

                elif noise_type == 'replace':
                    # Randomly replace a character with a random letter from the alphabet
                    idx = random.randint(0, len(noisy_word) - 1)
                    noisy_word[idx] = random.choice(alphabet)
                    ...
                    # Randomly replace a character with adjacent keys on a keyboard
                    idx = random.randint(0, len(noisy_word) - 1)
                    char = noisy_word[idx]
                    if char in keyboard_mappings:
                        noisy_word[idx] = random.choice(keyboard_mappings[char])
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for tokenized_sentence, tags in train_iterator:
                ...
                augmented_sentence = introduce_noise(tokenized_sentence, aug_ratio=0.2)  #Augmenting Data before batching
