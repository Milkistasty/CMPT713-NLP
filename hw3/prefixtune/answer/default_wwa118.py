...
from peft import PeftModel, PeftConfig, get_peft_model, PrefixTuningConfig
from tqdm import tqdm
...
class TableToText:
    ...
    def predict(self, model, src, num_sequences=1):
        # Uncomment these 2 lines if you want to also include the input in our generated output
        ...
        with torch.no_grad():
            ...
            outputs = model.generate(
                ...
                # need to check this score with different generated texts, to see if they are very different or just uniformly distributed
                output_scores=True, 
            ...
            # # generated_text = text.lstrip().replace(self.prompt + src, "").replace("\n", " ")
    ...
    # These 3 functions are for the post-decoding filter
    ...
    # def contains_table_info(self, generated_text, src):
    #     src_fields = [field.strip().split(":")[0] for field in src.split("|")]
    #     for field in src_fields:
    #         if field.lower() in generated_text.lower():
    #             return True
    #     return False
    
    # def custom_score(self, generated_text, score, src):
    #     repetition_penalty = -1.0 if self.has_repetition(generated_text) else 0.0
    #     content_reward = 1.0 if self.contains_table_info(generated_text, src) else 0.0
    #     return score + repetition_penalty + content_reward
...
if __name__ == '__main__':
    ...
    if os.path.isdir(modelfile + opts.modelsuffix) and not opts.force:
        print(f"Found modelfile {modelfile + opts.modelsuffix}. Starting decoding.", file=sys.stderr)
        model = AutoModelForCausalLM.from_pretrained(opts.basemodel)
        model = PeftModel.from_pretrained(model, modelfile + opts.modelsuffix)
        model = model.to(device)
